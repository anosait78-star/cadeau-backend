...
| Encryption  | `ENCRYPTION_KEY`, `PII_HASH_KEY`             | Secrets manager; separate keys, separate schedules. Rotating `ENCRYPTION_KEY` requires a re-encryption plan; rotating `PII_HASH_KEY` requires a re-hash migration ([privacy-model.md](privacy-model.md) §3). |
